...
#define DS1302_RAMSTART				0xC0 	//   RAM


/* USER CODE BEGIN PD */
//  ? BCDDS13020-99
#define HEX2BCD(v)	((v) % 10 + (v) / 10 * 16)  // BCDDS1302
#define BCD2HEX(v)	((v) % 16 + (v) / 16 * 10)  // BCDDS1302
/* USER CODE END PD */
...
{	
    // 
    if (buf[1] > 99) buf[1] = 99;    // 0-99
    if (buf[2] < 1 || buf[2] > 12) buf[2] = 1; // 1-12
    if (buf[3] < 1 || buf[3] > 31) buf[3] = 1; // 1-31
    if (buf[4] > 23) buf[4] = 23;    // 0-23
    if (buf[5] > 59) buf[5] = 59;    // 0-59
    if (buf[6] > 59) buf[6] = 59;    // 0-59
    if (buf[7] < 1 || buf[7] > 7) buf[7] = 1; // 1-7

    DS1302_WriteByte(DS1302_CONTROL, 0x00);			// 
    delayUS_DWT(20);
    DS1302_WriteByte(DS1302_SEC, 0x80); // DS1302
    DS1302_WriteByte(DS1302_YEAR, HEX2BCD(26));
    DS1302_WriteByte(DS1302_MONTH, HEX2BCD(buf[2])); // buf[2]120x12
    ...
    DS1302_WriteByte(DS1302_SEC, HEX2BCD(buf[6])); // 0x80
...
void DS1302_ClockStart(void)
{
	...
	buf = DS1302_ReadByte(DS1302_SEC) & 0x7F;		//  8 
